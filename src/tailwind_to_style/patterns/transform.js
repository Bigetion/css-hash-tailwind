const transform = {
  ...
  perspectiveOrigin: {
    regex: /^perspective-origin-(top|right|bottom|left|center|top-right|top-left|bottom-right|bottom-left)$/,
  ...
  transformOrigin: {
    regex: /^origin-(top|right|bottom|left|center|top-right|top-left|bottom-right|bottom-left)$/,
  ...
  translate: {
    ...
    cssProp: "transform",
    formatter: (x, y) =>
      `translate(${x.endsWith("px") ? x : `${x}px`}, ${
        y.endsWith("px") ? y : `${y}px`
      })`,
  },
  translate3d: {
    regex: /^translate-3d-(.+)$/,
