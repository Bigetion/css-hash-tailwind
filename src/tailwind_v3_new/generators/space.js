    ...
    const generateDirectionalSpace = (direction, key, value) => {
      ...
      return `
        ${pseudoClass((ps) => selector('', ps), variantOptions)} {
          ...
          margin-${properties.first}: calc(${value} * calc(1 - var(--space-${properties.axis}-reverse)));
          margin-${properties.second}: calc(${value} * var(--space-${properties.axis}-reverse));
        }
        ${pseudoClass((ps) => selector('-', ps), variantOptions)} {
          --space-${properties.axis}-reverse: 0;
          margin-${properties.first}: calc(-${value} * calc(1 - var(--space-${properties.axis}-reverse)));
