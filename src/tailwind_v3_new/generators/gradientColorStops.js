...
export default function generator(configOptions = {}) {
  ...
  function generateGradientStop(type, variantOptions, getCssByColors, pseudoClass) {
    return getCssByColors(gradientColorStops, (key, value, rgbValue) => {
      ...
      if (type === "from") {
        colorStopsCSS = `--gradient-color-stops: var(--gradient-from-color),var(--gradient-to-color,rgba(${rgbValue || "255,255,255"},0));`;
      } else if (type === "via") {
        colorStopsCSS = `--gradient-color-stops: var(--gradient-from-color),var(--gradient-via-color),var(--gradient-to-color,rgba(${rgbValue || "255,255,255"},0));`;
      } else if (type === "to") {
        colorStopsCSS = `--gradient-color-stops: var(--gradient-from-color),var(--gradient-to-color,rgba(${rgbValue || "255,255,255"},0));`;
      }
  ...
  return generateCssString(
    ({ pseudoClass, getCssByColors }) => {
      ...
      const fromClasses = generateGradientStop("from", variantOptions, getCssByColors, pseudoClass);
      const viaClasses = generateGradientStop("via", variantOptions, getCssByColors, pseudoClass);
      const toClasses = generateGradientStop("to", variantOptions, getCssByColors, pseudoClass);
